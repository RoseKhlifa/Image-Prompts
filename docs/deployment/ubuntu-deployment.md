...
CREATE DATABASE imageprompts OWNER imageprompts ENCODING 'UTF8' LC_COLLATE='zh_CN.UTF-8' LC_CTYPE='zh_CN.UTF-8' TEMPLATE template0;
...
> 如果 `LC_COLLATE='zh_CN.UTF-8'` 报错,先 `sudo locale-gen zh_CN.UTF-8` 再重试,或者直接用 `'en_US.UTF-8'` 也行,不影响业务。
...
新建一个非 root 用户跑应用(安全考虑):

```bash
sudo adduser --disabled-password --gecos "" imageprompts
sudo su - imageprompts
```

以下所有操作都在 `imageprompts` 用户下:
...
# ── 站点资源根目录(import 用) ────────────────
IMPORT_DATA_ROOT=/home/imageprompts/import-data   # 后面把要导入的 JSONL 放这里
```
...
```bash
sudo nano /etc/systemd/system/image-prompts-api.service
```
...
Type=simple
User=imageprompts
WorkingDirectory=/home/imageprompts/Image-Prompts
Environment=NODE_ENV=production
EnvironmentFile=/home/imageprompts/Image-Prompts/apps/api/.env
ExecStart=/usr/bin/node /home/imageprompts/Image-Prompts/node_modules/.pnpm/tsx@*/node_modules/tsx/dist/cli.mjs apps/api/src/index.ts
Restart=always
RestartSec=5
...
> 如果 `apps/api` 有编译产物(比如 `dist/index.js`),把 `ExecStart` 换成 `/usr/bin/node /home/imageprompts/Image-Prompts/apps/api/dist/index.js` 更稳。

启动:

```bash
sudo systemctl daemon-reload
sudo systemctl enable --now image-prompts-api
sudo systemctl status image-prompts-api    # 看日志
sudo journalctl -u image-prompts-api -f    # 实时跟
```
...
3. 代理目标:
   ...
   - `/` → 静态目录 `/home/imageprompts/Image-Prompts/apps/web/dist`
...
4. **网站设置**:`/zh/rosekhlifa/config` 改 `import.data_root` 为 `/home/imageprompts/import-data`(如果要在 production 上传 JSONL)
...
```bash
# 本地
scp -r G:/promptsandimages/exports/* imageprompts@<vps-ip>:/home/imageprompts/import-data/
```
