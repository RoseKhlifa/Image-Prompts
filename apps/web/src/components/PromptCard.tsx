...
export default function PromptCard({ prompt }: { prompt: PromptSummary }) {
  ...
  return (
    <Link
      ...
      className="group relative mb-1 block overflow-hidden rounded-card border border-border-soft bg-panel transition-colors hover:border-accent/40 break-inside-avoid"
    >
      <img
        src={imageUrl}
        alt={title}
